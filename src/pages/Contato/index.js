...
class Contato extends Component {
  ...
  handleRemover(){
    let key = "-Lf1av4Eiel7wjIamQj9"
    firebase.database().ref('Contato/').child(key).remove()
    Alert.alert("Contato Removido Com Sucesso")
    }
  handleAtualizar(){
    if(this.state.nome != ""){
      firebase.database().ref('Contato/-Lf1dUMF0Idq2cSJLxRn').update({
        ...
        <FlatList 
         ...
         <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
           ...
           <TouchableOpacity onPress={() => this.setModalVisible(true)}>
             <Image source={edit} style={{marginRight: 5, marginTop: 5}}/>
           </TouchableOpacity>
         </View>
         <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
           <Text style={styles.textEmail}>{item.email}</Text>
           <TouchableOpacity onPress={this.handleRemover}>
              <Image source={remove} style={{marginTop: 10, marginRight: 5}}/>
           </TouchableOpacity>
         </View>
       </View>}
        />
...
//         <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
//           <Text style={styles.textNome}>{this.props.data.nome}</Text>
//           <TouchableOpacity onPress={() => this.refs.modal2.open()}>
//             <Image source={edit} style={{marginRight: 5, marginTop: 5}}/>
//           </TouchableOpacity>
//         </View>
//         <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
//           <Text style={styles.textEmail}>{this.props.data.email}</Text>
//           <Image source={remove} style={{marginTop: 10, marginRight: 5}}/>
//         </View>
//       </View>
//     )
